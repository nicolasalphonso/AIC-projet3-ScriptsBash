...
do
	...
	sshpass -p administrateur ssh -tt -o StrictHostkeyChecking=no administrateur@$adresseAbeille << 'EOF'
		crontab -r
		(crontab -l 2>/dev/null; echo "08 16 * * 2 /home/administrateur/scriptInitialisation.sh") | crontab -
		exit
EOF
...
do
	...
	sshpass -p administrateur ssh -o StrictHostkeyChecking=no administrateur@$adresseBaobab << 'EOF'
		crontab -r
		(crontab -l 2>/dev/null; echo "08 16 * * 2 /home/administrateur/scriptInitialisation.sh") | crontab -
		exit
EOF
